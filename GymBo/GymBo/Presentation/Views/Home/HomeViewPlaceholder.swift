...
struct HomeViewPlaceholder: View {
    ...
    init() {
        print("🚀🚀🚀 HomeViewPlaceholder INIT CALLED 🚀🚀🚀")
    }

    var body: some View {
        let _ = print(
            "🔍 HomeView body: hasActiveSession = \(sessionStore.hasActiveSession), currentSession = \(sessionStore.currentSession?.id.uuidString ?? "nil")"
        )
        NavigationStack {
            VStack(spacing: 32) {
                Spacer()
                ...
                let _ = print(
                    "🔍 HomeView: Evaluating button - hasActiveSession = \(sessionStore.hasActiveSession)"
                )
            ...
            .task {
                print("🔍 HomeView.task - Loading active session")
                await sessionStore.loadActiveSession()
                print(
                    "🔍 HomeView.task - After load: hasActiveSession = \(sessionStore.hasActiveSession)"
                )
            }
            .onAppear {
                print("🔍 HomeView.onAppear - hasActiveSession: \(sessionStore.hasActiveSession)")
                print("   - currentSession: \(sessionStore.currentSession?.id.uuidString ?? "nil")")
                print("   - exercises count: \(sessionStore.currentSession?.exercises.count ?? 0)")
            }
        }
    }

...
export function statusPage({ title, message, success = false }) {
  return `<!doctype html>
<html lang="de">
<head>
  <meta charset="utf-8">
...
<body><main><h1>${escapeHtml(title)}</h1><p>${escapeHtml(message)}</p><a href="/blog/">Zum Blog</a></main></body>
</html>`;
}

export function sendStatus(res, statusCode, payload, request) {
